...
out_dir2 = "./R_out/R05_DA_DEG_deseq_by_cluster_Seur_integr/"
if (!dir.exists(out_dir2)) {dir.create(out_dir2)}

#load group and file info
gr_tab = read_csv("R_in/group_tab_rcs.csv")

#get marker gene panels
marker_tab = read_csv("R_in/cell type markers 22-02-23.csv")
GOI = list()
t1 = read_csv("R_in/Transcription Factors hg19 - Fantom5_21-12-21.csv")
GOI$TF = t1$Symbol 
t1 = read_csv("R_in/HSA21_genes_biomaRt_conversion.csv")
GOI$HSA21 = na.omit(t1$hgnc_symbol)

#load processed/integrated dataset
load(file = "./R_out/R04_cluster_char_Seur_integr/R04_seur_clean_clustered.rda") 

# load cluster assignment (clusters not labelled here)
clust_assign_tab = read_csv("R_out/R04_cluster_char_Seur_integr/R04_cluster_assignment_cleaned.csv")
...
t1 = clust_assign_tab
seur$cluster_name = t1$cluster_name[match(seur$seurat_clusters, t1$cluster)]
seur$cell_class = t1$cell_class[match(seur$seurat_clusters, t1$cluster)]
...
clusters = unique(seur$seurat_clusters)
cluster_names = unique(seur$cluster_name)
clust_colors = matlab.like(length(clusters))

cell_classes = unique(seur$cell_class)
class_colors = matlab.like(6)
if (length(cell_classes)>5){class_colors = matlab.like(length(cell_classes))}


### plot UMAP and PCA plots by cluster_name and cell_class
DefaultAssay(seur) = "integrated"

pl = list()

pl[["clusters_umap"]] = DimPlot(seur, group.by = "cluster_name", 
                                label = TRUE, cols = clust_colors, reduction = "umap", shuffle = TRUE,
                                pt.size = 0.01)+ NoLegend()
pl[["cell_class_umap"]] = DimPlot(seur, group.by = "cell_class", 
                                label = TRUE, cols = class_colors, reduction = "umap", shuffle = TRUE,
                                pt.size = 0.01)+ NoLegend()
pl[["groups"]] = DimPlot(seur, reduction = "umap", group.by = "group", shuffle = TRUE,
                         cols = gr_colors, pt.size = 0.01)
pl[["samples"]] = DimPlot(seur, reduction = "umap", group.by = "sample", 
                          cols = sample_colors, pt.size = 0.01)
pl[["clusters_PC1_2"]] = DimPlot(seur, group.by = "cluster_name", shuffle = TRUE,
                                 label = TRUE, cols = clust_colors, reduction = "pca", dims = c(1,2),
                                 pt.size = 0.01)
pl[["clusters_PC3_4"]] = DimPlot(seur, group.by = "cluster_name", shuffle = TRUE,
                                 label = TRUE, cols = clust_colors, reduction = "pca", dims = c(3,4), 
                                 pt.size = 0.01)
pl[["cell_class_PC1_2"]] = DimPlot(seur, group.by = "cell_class", shuffle = TRUE,
                                 label = TRUE, cols = class_colors, reduction = "pca", dims = c(1,2),
                                 pt.size = 0.01)
pl[["cell_class_PC3_4"]] = DimPlot(seur, group.by = "cell_class", shuffle = TRUE,
                                 label = TRUE, cols = class_colors, reduction = "pca", dims = c(3,4), 
                                 pt.size = 0.01)
pl[["sample_PC1_2"]] = DimPlot(seur, group.by = "sample", shuffle = TRUE,
                                   label = TRUE, cols = sample_colors, reduction = "pca", dims = c(1,2),
                                   pt.size = 0.01)
pl[["sample_PC3_4"]] = DimPlot(seur, group.by = "sample", shuffle = TRUE,
                                   label = TRUE, cols = sample_colors, reduction = "pca", dims = c(3,4), 
                                   pt.size = 0.01)

pdf(file = paste0(out_dir2,"R05_UMAP_by_cluster_name_cell_class.pdf"), width = 6.5, height = 4.5)
lapply(pl, function(x){x})
dev.off()



### plot dotplot with gene expression of marker genes by cluster_name
DefaultAssay(seur) = "SCT"

p1 = DotPlot(seur, features = intersect(marker_tab$gene, rownames(seur)), 
             group.by = "cluster_name", scale.by = "size") +RotatedAxis()

pdf(file = paste0(out_dir2,"/R05_marker_expr_dotplot_cluster_name.pdf"), width = 12, height = 7)
plot(p1)
dev.off()


### plot dotplot with gene expression of marker genes by cell_class

p1 = DotPlot(seur, features = intersect(marker_tab$gene, rownames(seur)), 
             group.by = "cell_class", scale.by = "size") +RotatedAxis()

pdf(file = paste0(out_dir2,"/R05_marker_expr_dotplot_cell_class.pdf"), width = 12, height = 4)
plot(p1)
dev.off()


### plot UMAP with gene expression of marker genes

DefaultAssay(seur) = "SCT"

#subsample seurat object for large datasets (else plots become too large (pdf with hundreds of MB))
if (nrow(seur@meta.data)>5000){
  seur.sub = seur[, sample(colnames(seur), size =5000, replace=F)]
} else {seur.sub = seur}


p1 = FeaturePlot(seur.sub, features = marker_tab$gene, order = TRUE,
                 reduction = "umap", pt.size = 0.01, ncol = 6)

pdf(file = paste0(out_dir2,"R05_marker_expr_UMAP.pdf"), width = 16, height = 16)
plot(p1)
dev.off()
...
stat_tab = stat_tab[order(stat_tab$cluster),]
...
plot_heatmap = function(mat, genes = rownames(mat), cluster_rows = FALSE,
  ...
  pl1 = pheatmap(m2, show_rownames=TRUE, cluster_rows = cluster_rows,
                 ...
                 border_color = NA, fontsize = fontsize, fontface = 2,
...
plot_cluster_diff_expr = function(bulk_data, pl_genes, 
      ...
      width = ncol(m_sample)/3+2, height = nrow(m_sample)*cellheight/40+2)
  {
    ...
    plot_heatmap(l1$DELTA, genes = pl_genes_clust, cluster_rows = FALSE,
