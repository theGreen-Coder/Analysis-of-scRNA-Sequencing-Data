...
plot_cluster_diff_expr = function(bulk_data, pl_genes, 
      ...
      width = ncol(m_sample)/3+2, height = nrow(m_sample)*cellheight/40+2)
  {
    ...
    write.csv(pl_genes_clust, "./significantGenes27Apr.csv", row.names=FALSE)
    ...
    write.csv(dataframePlot, "./geneOrderDataframe27Apr.csv", row.names=TRUE)
    plotDifferenceOrder = plotClusterDifference$tree_row$labels[plotClusterDifference$tree_row$order]
    write.csv(plotDifferenceOrder, "./geneOrderDifference27Apr.csv", row.names=FALSE)
